...
class TestHellerEinsteinBootstrap(unittest.TestCase):
    ...
    def test_identifier_reservations_exist_and_namespace_clean(self) -> None:
        ...
        self.assertIn("HE-PROJ-INV-004", text)
        self.assertIn("HE-PROJ-INV-005", text)
    ...
    def test_markov_shift_identifiability_theorem_exists(self) -> None:
        path = ROOT / "docs" / "projection" / "HE-PROJ-INV-003-markov-shift-identifiability.md"
        self.assertTrue(path.exists(), "HE-PROJ-INV-003 theorem document missing")
        text = path.read_text(encoding="utf-8")
        self.assertIn("HE-PROJ-INV-003", text)
        self.assertIn("two-sided Markov shift", text)
        self.assertIn("observer-generated", text)
